...
if (!in_array($closeMode, ['auto', 'immediate'], true)) {
    st_redirect_with_flash('cad_ticket', 'danger', 'Invalid close mode.', $redirectBack);
}
...
try {
    ...
    if ($closeMode === 'immediate') {
        ...
        st_insert_trail($conn, $ticketId, 'close', $userId, 'CAD', null, 'Ticket closed immediately.', null);
        st_insert_trail($conn, $ticketId, 'message', null, 'SYSTEM', null, 'Ticket has been closed by CAD.', ['automation' => true]);

        $conn->commit();
        $conn->autocommit(true);
        st_redirect_with_flash('cad_ticket', 'success', 'Ticket closed immediately.', $redirectBack);
    }
    ...
    $conn->commit();
    $conn->autocommit(true);
    st_redirect_with_flash('cad_ticket', 'success', 'Ticket marked for auto-close.', $redirectBack);
} catch (Exception $e) {
    $conn->rollback();
    $conn->autocommit(true);
