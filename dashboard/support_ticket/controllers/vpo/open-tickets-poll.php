...
function st_status_class_vpo_poll($status)
{
    return 'st-status st-status-' . strtolower((string) $status);
}

function st_partner_name_vpo_poll($ticket)
{
...
$badgeCounts = st_get_ticket_badge_counts($conn, $ticketNumbers, 'VPO');
...
<div class="st-ticket-table" role="table" aria-label="Open VPO tickets">
    ...
    <?php foreach ($vpoOpen as $ticket): ?>
        <button type="button" class="st-ticket-row" role="row" data-ticket-modal="stTicketTrailModalVpo-<?php echo (int) $ticket['id']; ?>" data-ticket-id="<?php echo (int) $ticket['id']; ?>" data-ticket-number="<?php echo htmlspecialchars((string) $ticket['ticket_number']); ?>" data-status="<?php echo htmlspecialchars((string) ($ticket['status'] ?? '')); ?>" data-seen-role="VPO">
            <?php $vpoUnread = (int) ($badgeCounts[(string) ($ticket['ticket_number'] ?? '')] ?? 0); ?>
            ...
            <span class="st-ticket-col st-col-partner"><?php echo htmlspecialchars(st_partner_name_vpo_poll($ticket)); ?></span>
            <span class="st-ticket-col st-col-status"><span class="<?php echo htmlspecialchars(st_status_class_vpo_poll($ticket['status'])); ?>"><?php echo htmlspecialchars((string) $ticket['status']); ?></span></span>
        </button>
    <?php endforeach; ?>
</div>
<?php
