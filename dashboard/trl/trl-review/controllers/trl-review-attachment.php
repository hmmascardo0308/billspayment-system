...
$canReview = !empty($userId) && function_exists('has_any_permission') &&
    has_any_permission(['TRL Review', 'Bills Payment']);
...
$stmt = $conn->prepare("SELECT a.file_name, a.mime_type, a.file_size, a.file_data
    ...
    WHERE a.id = ? AND (t.status IS NULL OR t.status = 'PENDING_APPROVAL')
    LIMIT 1");
if (!$stmt) {
...
if (!$attachment) {
    ...
    exit('Attachment not found or the transaction is no longer available for review.');
}
