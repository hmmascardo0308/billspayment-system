                ...
                if (isKP7) {
                    ...
                    for (let r = 9; r <= range.e.r; r++) {
                        ...
                        let partnerIdKpx = worksheet[XLSX.utils.encode_cell({r: r, c: 20})] ? 
                            String(worksheet[XLSX.utils.encode_cell({r: r, c: 20})].v).trim() : null;
                        ...
                        let rowData = {
                            ...
                            partner_id: partnerIdFallback, // This is column V (fallback) - ORIGINAL VALUE
                            partner_id_fallback_original: partnerIdFallback, // PRESERVE ORIGINAL VALUE
                            partner_name: null,
                            mpm_gl_code: null,
                            settle_unsettle: 'Unsettle',
                            claim_unclaim: null,
            ...
            success: function(response) {
                ...
                if(response.status === 'success') {
                    ...
                    allRows.forEach((row, index) => {
                        ...
                        const partnerIdFallback = row.partner_id_fallback_original !== undefined && row.partner_id_fallback_original !== null && row.partner_id_fallback_original !== '' 
                            ? String(row.partner_id_fallback_original).trim() 
                            : '';
                        
                        // Determine display value - prefer U, fallback to V
                        ...
                        const isBothEmpty = partnerIdKpx === '' && partnerIdFallback === '';
                        ...
                        if (isBothEmpty) {
                            ...
                            // Has some partner ID value (from U or V)
                            if (!isPartnerRecognized) {
                                // Partner ID exists but not found in masterfile
                                let displayValue = partnerIdKpx !== '' ? partnerIdKpx : partnerIdFallback + ' (from V)';
