...
// Function to get duplicate rows from the database
function getDuplicateRows($conn)
{
    $query = "SELECT MIN(id) AS id, status, cancellation_date, date_time, control_number, reference_number, payor, address, account_number, account_name, 
              ...
              WHERE reference_number IN (
                  SELECT reference_number FROM temp_billsPayment GROUP BY reference_number HAVING COUNT(*) > 1
              )
...
// Function to export duplicate rows to CSV file
function exportDuplicatesToCSV($conn)
{
    $result = getDuplicateRows($conn);
    if ($result->num_rows > 0) {
         // Generate filename with current date
         $filename = 'duplicate_transactions_' . date('Y-m-d') . '.csv';
        ...
        readfile($filename);
        exit; // Stop further execution after the download
    } else {
