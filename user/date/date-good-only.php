...
if (isset($_POST['export-good-only'])) {
    $from_date = $_POST['fromDate'];
    $to_date = $_POST['toDate'];
    $partner_id = ($_POST['partner'] != '') ? $_POST['partner'] : '%'; // '%' will select all partners if not specified

    $query = "SELECT * FROM billsPayment WHERE date_time >= '$from_date' AND date_time <= '$to_date' AND partner_id LIKE '$partner_id'";
    $query_run = mysqli_query($conn, $query);

    // Export the data to CSV
    $filename = 'Bills Payment Good Only Transactions_' . date('Y-m-d') . '.csv';
        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                ...
                <div class="card-header">
                        ...
                        <center><h4>Bills Payment Transaction (Good Only)</h4></center>
                    </div>
                    <div class="card-body">
                            ...
                            <form action="" method="POST">
                                ...
                                <div class="col-md-4">
                                    <div class="form-btn">
                                        ...
                                        <input type="submit" name="export-good-only" id="export" value="Export to CSV" class="btn btn-info" onclick="openFileExplorer(event)" />
                                    </div>
                                </div>
                            </div>
                        </form>
                    ...
                    <?php 
                        ...
                        if(isset($_POST['fromDate']) && ($_POST['toDate'])) {
                            ...
                            $to_date = $_POST['toDate'];
                            $partner_id = ($_POST['partner'] != '') ? $_POST['partner'] : '%'; // '%' will select all partners if not specified
                            $query = "SELECT * FROM billsPayment WHERE date_time >= '$from_date' AND date_time <= '$to_date' AND status != '*' AND partner_id LIKE '$partner_id'";
                            $query_run = mysqli_query($conn, $query);
