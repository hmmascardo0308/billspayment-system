...
$filename = isset($_POST['filename']) ? $_POST['filename'] : 'empty_cell_errors';
...
$pdf->SetTitle('Empty Cell Error Report');
$pdf->SetSubject('Empty Cell Errors');
$pdf->SetKeywords('Cell, Empty, Error, Report');
...
// Title
$pdf->Cell(0, 10, 'Empty Cell Error Report', 0, 1, 'C');
...
$pdf->MultiCell(0, 10, 'The following rows have empty values in the branch ID cell (Column N):', 0, 'L', 0);
...
$pdf->Cell(60, 8, 'ML Branch Outlet', 1, 0, 'C', 1);
...
foreach ($errors as $index => $error) {
    ...
    $pdf->Cell(60, 8, $error['outlet'], 1, 0, 'L', $fill);
...
$pdf->MultiCell(0, 8, 'Please fill in the empty branch ID cells in column N before importing. Missing branch IDs will prevent proper transaction processing and reporting.', 0, 'L', 0);

// Output the PDF
$pdf->Output('empty_cell_errors.pdf', 'D');
exit;
?>
