/**
 * Branch ID Error Handling Functions
 * Contains the print and export functionality for branch ID error reports
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    filenamePHP = document.getElementById('original_filename') ? 
        document.getElementById('original_filename').value : 'branch_id_errors';
    sourceFileTypePHP = document.getElementById('source_file_type') ? 
        document.getElementById('source_file_type').value : 'Unknown';
    transactionDatePHP = document.getElementById('transaction_date') ? 
...
function printReport() {
    try {
        ...
        var htmlContent = '<!DOCTYPE html>' +
            '<html>' +
            '<head>' +
            '    <title>Branch ID Error Report</title>' +
            '    <style>' +
            ...
            '    </div>' +
            '    <div class="message">' +
            '        The following branch IDs were not found in the branch profile database:' +
            '    </div>' +
            ...
            '                    <th style="width: 32%;">ML Branch Outlet</th>' +
        ...
        htmlContent +=
            ...
            '    </div>' +
            '    <div class="footer">' +
            '        <p>Please correct these branch IDs in your file before importing.</p>' +
            '        <p>Missing branch IDs will prevent proper transaction processing and reporting.</p>' +
            '    </div>' +
            '    <script>' +
...
/**
 * Export branch ID errors to PDF
 */
function exportToPDF() {
    try {
        ...
        form.method = "post";
        form.action = "branchIdErrorExportToPdf.php";
